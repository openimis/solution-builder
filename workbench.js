...
async function ensureDistDkrRepo(branch = "develop") {
  ...
  if (!fs.existsSync(distDkrRepoPath)) {
    ...
    await simpleGit().clone(
      "https://github.com/openimis/openimis-dist_dkr.git",
      distDkrRepoPath
    );
  }
...
async function copyDistDkrAssetsFromCompose(
  composeFilePath,
  branch = "develop"
) {
  if (!fs.existsSync(composeFilePath)) {
    console.warn(
      "⚠️ compose.yml not found in the solution folder, skipping..."
    );
    return {};
  }
...
async function main() {
  try {
    ...
    const composeFiles = await copyDistDkrAssetsFromCompose(
      composeFilePath,
      "develop"
    );

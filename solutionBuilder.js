...
async function fetchJSON(handleOrUrl, DIRECTORY = null, rootPath = "") {
  ...
  if (isHttpUrl(handleOrUrl)) {
    ...
    if (!response.ok)
      throw new Error(`Failed to fetch ${handleOrUrl}: ${response.statusText}`);
...
function getBePackageConf(name, definition, branch) {
  ...
  if (branch == "released") {
    ...
    pip =
      "git+" + definition.git + ".git@" + branch + "#egg=" + definition.package;
  }
...
async function processSolutions(
  ...
  solutionFilePath = getAbsolutePath(
  ...
  merged.bePackagesList = merged.bePackagesList.filter(
    (item, index) => merged.bePackagesList.indexOf(item) === index
  );
  for (let idx in merged.bePackagesList) {
    bePackage = merged.bePackagesList[idx];
    PIPModules.add(
      getBePackageConf(bePackage, merged.bePackagesDefDict[bePackage], branch)
    );
  }
  let NPMModules = new Set();
  merged.fePackagesList = merged.fePackagesList.filter(
    (item, index) => merged.fePackagesList.indexOf(item) === index
  );
  for (let idx in merged.fePackagesList) {
    fePackage = merged.fePackagesList[idx];
    NPMModules.add(
      getFePackageConf(fePackage, merged.fePackagesDefDict[fePackage], branch)
    );
  }
  let services = {};
  for (let idx in merged.servicesList) {
    service = merged.servicesList[idx];
    services = getServiceConf(
      service,
      merged.servicesDefDict[service],
      services
    );
  }
...
async function mergeSolutions(
  ...
  menusDict = {},
  moduleRefDict = {},
  bePackagesList = new Set(),
  bePackagesDefDict = {},
  fePackagesList = new Set(),
  fePackagesDefDict = {},
  ...
  if (typeof window === "undefined" && typeof solutionFile === "string") {
    ...
    solutionFile == "[object FileSystemFileHandle]"
  ) {
...
async function createZip(data, filename) {
  ...
  try {
    if (typeof window === "undefined") {
      // Node.js: Generate ZIP as nodebuffer
      const content = await zip.generateAsync({ type: "nodebuffer" });
      fs.writeFileSync(filename, content);
      console.log(`ZIP file "${filename}" created successfully on disk!`);
    } else {
      // Browser: Generate ZIP as blob and trigger download
